...
Rule ParseRule(char str[], int len) {
    ...
    // 注意，item可能为空，即只有一个斜杠。
    if (str[len - 1] == '/') { r.endslash = (r.item.empty() || r.item.back().type != PATH); }
    return r;
}
...
void PrintRule(Rule& r) {
    for (int i = 0; i < r.item.size(); ++i) {
        ...
        Node& n = r.item[i];
        switch (n.type) {
            case LIT: printf("%s", n.val.c_str()); break;
            default: printf("<%s>", Name[n.type]); break;
        }
    }
    if (r.endslash) { putchar('/'); }
    puts("");
}
...
bool Match(Rule& r, char str[], int len) {
    ...
    */
    ...
    //(int)r.endslash,j,len);

    if (j == len) { return !r.endslash; }
    if (j == len - 1) {
        bool endslash = (str[j] == '/');
...
int Match2(char str[], int len) {
    ...
    for (int i = 0; i < rules.size(); ++i) {
        if (Match(rules[i], str, len)) { return i; }
    }
    return -1;
}
