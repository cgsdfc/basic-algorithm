...
bool Create(string& str, ll size) {
    ...
    if (i == Path.size()) {
        ...
        assert(fa->dir);
        // 注意配额==0时，表示无限。
        if (fa && fa->Q1 && fa->size1 + diff > fa->Q1) { return false; }
        // 检查祖先节点size2.
        while (fa) {
            if (fa->Q2 && fa->size2 + diff > fa->Q2) { return false; }
            fa = fa->father;
        }
        // 正式修改文件系统。
    ...
    Node* fa = p;
    while (fa) {
        if (fa->Q2 && fa->size2 + size > fa->Q2) { return false; }
        fa = fa->father;
    }
    // 配额满足，正式修改文件系统。
    // 先补充目录。
...
bool Quota(string& str, ll Q1, ll Q2) {
    ...
    */
    ...
    // 不能把配额改的小于当前大小。
    // 注意配额为0表示无限。
    if ((Q1 && p->size1 > Q1) || (Q2 && p->size2 > Q2)) { return false; }
    p->Q1 = Q1;
    p->Q2 = Q2;
    return true;
}
