...
struct Time {
  ...
  void Print() { printf("%04d%02d%02d%02d%02d", y, mon, d, h, min); }
  friend bool operator==(const Time &a, const Time &b) {
    return a.y == b.y && a.mon == b.mon && a.d == b.d && a.h == b.h &&
           a.min == b.min;
  }
  void inc() {
    // 自增1分钟。
...
struct Node {
  ...
  bool Match(const Time &t) {
    return conds[0].Match(t.min) && conds[1].Match(t.h) &&
           conds[2].Match(t.d) && conds[3].Match(t.mon) && conds[4].Match(t.w);
  }
};
