...
int Func(vector<int>& in, int op) {
    ...
    for (int i = 1; i < in.size(); ++i) {
        ...
        switch (op) {
            case AND:
            case NAND: ans &= x; break;
            case OR:
            case NOR: ans |= x; break;
            case XOR: ans ^= x; break;
        }
    }
    switch (op) {
        case NOT:
...
int Parse(char str[]) {
#define TRAN(x) \
    if (!strcmp(#x, str)) return x;
    TRAN(NOT)
    TRAN(AND)
    TRAN(OR) TRAN(XOR) TRAN(NAND) TRAN(NOR)
#undef TRAN
        return -1;
}
