...
static void PushQueue(Queue q, ElemType item) {
...
static ElemType PopQueue(Queue q) {
    QNode* n;
    ElemType item;
...
void LevelOrderTraversal(Node* node) {
    ...
    while (!Empty(q)) {
        ...
        */
        node = PopQueue(q);
        // 处理node
        if (node->left) { PushQueue(q, node->left); }
        if (node->right) { PushQueue(q, node->right); }
    }
}
...
static void PrintTree2(Node* node, int ord) {
    if (node) {
        if (ord == 1) printf("%d ", node->data);
        PrintTree2(node->left, ord);
        if (ord == 2) { printf("%d ", node->data); }
        PrintTree2(node->right, ord);
        if (ord == 3) { printf("%d ", node->data); }
    }
}
