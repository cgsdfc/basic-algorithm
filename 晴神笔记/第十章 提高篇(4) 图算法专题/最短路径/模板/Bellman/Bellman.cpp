...
没有最短，只有更短）。存在可达负环是最短路不存在的充要条件。 注意，只要存在S可达的负环，
则对任意节点V来说，S到V的最短路都不存在。 
...
3. 算法主体有V-1轮，每轮计算出最短路树的一层的全部节点的距离，因为最短路树最多V层，
而第一层是根，即源点S，在初始化确定，故算法最多需要V-1次循环。实际上不一定需要V-1次。
4. 在主体循环后面，有一段检测负环的逻辑，决定了算法的返回值。为什么这样呢？因为如果最短路存在，
		...
		for (int u=0;u<N;++u) {
			for (int i=0;i<Adj[u].size();++i) {
				...
				if (d[u] + dis < d[v]) {
					...
					即 pre[v] 没有u，那么就是新路径。如果u已经存在于v的前驱中，那么看num[v]
