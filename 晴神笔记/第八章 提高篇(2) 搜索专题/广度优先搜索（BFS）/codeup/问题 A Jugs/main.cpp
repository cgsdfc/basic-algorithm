...
struct Node {
    ...
    */
    bool WrongLess(const Node& a) const {
        ...
        */
        for (int i = 0; i < N; ++i) {
            if (data[i].now < a.data[i].now) return true;
        }
        return false;
    }
    ...
    friend bool operator<(const Node& a, const Node& b) {
        for (int i = 0; i < N; ++i) {
            ...
            */
            if (a.data[i].now != b.data[i].now) { return a.data[i].now < b.data[i].now; }
        }
        return false;
    }
    // 返回Op是否能运行。
    bool CanDo(OP op, int i, int j) {
        switch (op) {
            case EMPTY: return data[i].now > 0;
            case FILL: return data[i].now < data[i].max;
            case POUR:
                // i => j
    ...
    void Do(OP op, int i, int j) {
        ...
        int v;
        switch (op) {
            case EMPTY: data[i].now = 0; break;
            case FILL: data[i].now = data[i].max; break;
            case POUR:
...
void PrintResult(vector<Oper>& v) {
    ...
    for (int i = 0; i < v.size(); ++i) {
        switch (v[i].op) {
            case EMPTY: printf("empty %s\n", To(v[i].i)); break;
            case FILL: printf("fill %s\n", To(v[i].i)); break;
            case POUR: printf("pour %s %s\n", To(v[i].i), To(v[i].j)); break;
        }
    }
    puts("success");
}
