...
const char WeekNames[][20]={{}, "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"};
const char MonNames[][20]={{},"Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"};
const int MonDays[]={0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};

int FindMon(char name[]) {
	int i;
	for (i=1;i<=12;++i) {
		if (memcmp(MonNames[i], name, 3) == 0) {
			return i;
		}
	}
	assert(0);
