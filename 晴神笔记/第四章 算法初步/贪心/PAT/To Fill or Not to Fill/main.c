...
/*
        ...
        2. 在A中查找距离最近且油价比now低的油站x，若找到，则在now加足够到
   x的油， （若当前油量足够则不用加）。
        ...
        解析：
        1.
   每次加油不必加满，因为当前的油不一定最便宜，如果总是加满，则错过了前往
        前面更便宜的加油站，加更便宜的油的机会。
        2.
   贪心体现在：每次寻找加油站时，选择的原则是寻找更低的油价，而不在乎是否要
        一次性行驶更长的距离。因为并不是加油的次数越少越好，相反，如果油站的分布是越到后面
        越便宜，则应该频繁加油，使得当前的油的单位价格更低。
        3.
   每次寻找油站，都在当前油量可达的范围内寻找，这一点我也是明白的，关键是在这些可达
        油站中选择的策略。
        4.
   从贪心原则2出发，如果可达油站存在油价低于当前油站的，则会使后续路段的成本下降，因此优先
        ...
        6.
   若4的情况下油多个油站比当前油站便宜，则应该去最近那个，因为去更远的油站，意味着要在当前油站（贵）多加，
        不符合贪心追求局部最优原则。
        7. 若5的情况，有多个最小值，怎么办？
   暂时想不出来，先假设这样的情况不出现，如果出现了，则选取距离最近的。
        8.
   注意，可达油站的搜索范围是满油状态下，因为出发前是要在now加油的，最多可以加满（并且最优策略也有加满
        的情况），因此不是当前油量作为搜索范围。
        */
